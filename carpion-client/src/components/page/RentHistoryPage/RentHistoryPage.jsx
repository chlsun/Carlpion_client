...
const RentHistoryPage = () => {
  ...
  useEffect(() => {
    if (auth.accessToken) {
      axios
        .get("http://localhost/rents/reservations", {
          headers: {
  ...
  useEffect(() => {
    if (auth.accessToken) {
      axios
        .get(`http://localhost/rents/history/${4 + limit}`, {
          headers: {
            Authorization: `Bearer ${auth.accessToken}`,
          },
        })
  ...
  return (
    <>
      <div id="rent-history">
        {reservationHistory.historyList.length == 0 &&
        reservationList.length == 0 ? (
          ...
          <>
            ...
            <div className="container">
                ...
                {reservationList.map((reservation) => (
                  <div className="rent-component" key={reservation.impUID}>
                    ...
                    <div className="box">
                      ...
                      <p className="date">
                        {changeDate(reservation.rentalDate)} ~{" "}
                        {changeDate(reservation.returnDate)} |{" "}
                        <b>
                          총{" "}
                          {getDiffHour(
                    ...
                    <div className="box">
                      ...
                      <p className="price">
                        <b>{reservation.totalPrice}원</b>
                      </p>
                    </div>
                    <div className="absolute">
                      {!getStatus(reservation.rentalDate) && (
                        <button
                          type="button"
                          className="cancel-btn"
                          onClick={() => DeleteHandler(reservation.impUID)}
                        >
                          예약 취소
                        </button>
                      )}
              ...
              {reservationHistory.historyList.map((history) => (
                <div className="rent-component" key={history.impUID}>
                  ...
                  <div className="box">
                    <p className="title">대여/반납일</p>
                    <p className="date">
                      {changeDate(history.rentalDate)} ~{" "}
                      {changeDate(history.returnDate)} |{" "}
                      <b>
                        총 {getDiffHour(history.rentalDate, history.returnDate)}
                        시간
                      </b>
                    </p>
                  </div>
              ...
              {reservationHistory.historyList.length <
                reservationHistory.historyCount && (
                <button
                  className="more-btn"
                  onClick={() => {
                    setLimit(limit + 4);
