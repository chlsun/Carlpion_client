...
import nrstyles from "./ReportReply.module.css";
...
function ReportReply({ reportNo }) {
      ...
      const fetchComments = async () => {
         ...
         try {
            const response = await axios.get(
               `${apiUrl}/reports/comments?reportNo=${reportNo}`,
               accessToken
                  ? {
                       headers: {
   ...
   const handleDelete = async (commentNo) => {
      try {
         await axios.delete(`${apiUrl}/reports/comments/${commentNo}`, {
            headers: {
               Authorization: `Bearer ${accessToken}`,
            },
         });
            ...
            <div className={nrstyles.commentList}>
               {comments.map((comment) => (
                  <CommentItem
                     ...
                     setOpenMenuId={setOpenMenuId} // 메뉴 상태 관리 함수 전달
                  />
               ))}
            </div>
...
function CommentItem({
   ...
   const handleMenuToggle = () => {
      setOpenMenuId(
         openMenuId === comment.commentNo ? null : comment.commentNo
      );
   };
      ...
      <div className={nrstyles.comment}>
         <div className={nrstyles.commentHeader}>
            ...
            <div className={nrstyles.rightTop}>
               <span className={nrstyles.commentTime}>
                  {comment.createDate}
               </span>
