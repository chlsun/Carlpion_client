...
const PaymentHistoryPage = () => {
   ...
   useEffect(()=>{
      if(auth.accessToken){
         axios.get(`http://localhost/rents/payment/${impUID}`,
            {
               headers: {
            ...
            const diffHours = hourCalculator(result.data.rentalDate, result.data.returnDate);
      ...
      <>
         <main id="rental-history">
            <div className="main-container">
               ...
               <button className="goto-main" onClick={()=>navi("/")}>메인페이지로 이동</button>
               <button className="goto-history" onClick={()=>navi("/rent-history")}>차량 렌트 내역 보기</button>
            </div>
         </main>
      </>
   );
