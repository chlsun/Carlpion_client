...
const Login = () => {
   ...
   const validateForm = () => {
      ...
      inputFields.forEach((field) => {
         ...
         if (
            inputValues[key] &&
            !validationRules[field.id].regExp.test(inputValues[key])
         ) {
   ...
   const handleSubmit = (e) => {
      ...
      setIsProgress(true);
      axios
         .post(`${apiUrl}/auth/login`, inputValues)
         .then((result) => {
            const {
               username,
               nickname,
            ...
            <div className="w-xl px-24 my-48 bg-white dark:bg-gray-800 border-2 border-maincolor rounded-2xl flex flex-col justify-center items-center">
                     ...
                     {inputFields.map((field) => (
                        <li key={field.id} className="flex flex-col">
                           ...
                           {fieldMessages[field.id] && (
                              <p
                                 className={`mt-1 ml-1 font-Pretendard text-lg text-red-500`}
                              >
                                 {isValid[field.id]
                                    ? ""
                                    : fieldMessages[field.id]}
               ...
               <section className="w-full mt-6 flex">
                  <button
                     ...
                     ref={keepBtnRef}
                     onClick={handleKeep}
                     className="size-7 mr-2 text-gray-300 border-2 border-gray-300 rounded-full"
                  >
                     ✓
                  </button>
                  <label
                     htmlFor="keep"
                     className="font-maintheme text-xl text-gray-500"
                  >
                     로그인 상태 유지
                  </label>
               </section>
               ...
               <section className="flex flex-col">
                  ...
                  <div className="mt-12 mb-4 flex justify-center font-maintheme text-xl text-gray-500 tracking-wider">
                     처음이신가요?
                  </div>
                  <div className="mb-24 flex items-center gap-4">
                     ...
                     <div className="font-maintheme text-lg text-gray-500 tracking-wider">
                        또는
                     </div>
                     <div className="flex items-center gap-2">
                        <button
                           ...
                           type="button"
                           className="size-12 border-2 border-gray-300 rounded-md cursor-pointer"
                        >
