...
const LoginRedirect = () => {
      ...
      if (code) {
         axios
            .post(`https://oauth2.googleapis.com/token`, {
               code: code,
               client_id:
                  "430311231437-p5htp79gao25qmgsqrt26t8q2hkjjuue.apps.googleusercontent.com",
   ...
   useEffect(() => {
      if (googleLoginInfo) {
         axios
            .post(`${apiUrl}/auth/login-social`, googleLoginInfo)
            .then((googleLoginRes) => {
               if (googleLoginRes.status === 200) {
                  const { socialId, platform, nickname, realname, email } =
                     googleLoginRes.data;
                  socialLogin(
                     socialId,
                     platform,
      ...
      <>
         <div className="size-full min-h-screen bg-gray-100 dark:bg-gray-900 flex justify-center select-none">
            <div className="w-xl px-24 my-48 bg-white dark:bg-gray-800 border-2 border-maincolor rounded-2xl flex flex-col justify-center items-center">
               <div className="font-maintheme text-4xl text-maincolor">
                  로그인 진행 중 입니다...
               </div>
            </div>
         </div>
